...
import skimage
from networkDesign3 import cnnModel3
import h5py
import os
...
def convolve_wildfire_train(files,modelFnc,epochs=100,batchSize=100,model_dir="../models/wf_model"):

    config = tf.estimator.RunConfig(log_step_count_steps=10,
                                    save_checkpoints_steps=100,
                                    save_summary_steps=100)
    # Create the Estimator
    classifier = tf.estimator.Estimator(model_fn=modelFnc, model_dir=model_dir,config=config)
    
    # Train the model
    '''
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
            x={'files': np.array(files),
               'batch_size': np.array([batchSize])},
            y=np.array([np.array(files),np.array([batchSize])]),
            num_epochs=None,
            shuffle=True)
    '''
    #train_input_fn = myInputFn(files,batchSize,epochs)
    classifier.train(input_fn=lambda:myInputFn(files,batchSize,epochs),steps=epochs)
    #classifier.train(input_fn=myInputFn,steps=epochs)
    #classifier.train(input_fn=lambda: myInputFn(),steps=epochs)
    
def convolve_wildfire_train_preload(datas,truths,modelFnc,epochs=100,batchSize=100,model_dir="../models/wf_model"):
...
def myInputFn(files,batchSize,epochs):
    #files = glob.glob(inDir+'run*.h5')
    #batchSize = 4
    #files = features
    #batchSize = batch_size
    dataset = tf.data.Dataset.from_tensor_slices((files)).shuffle(len(files))
    #dataset = dataset.map(lambda filename: tuple(tf.py_func(readSpecH5, [filename], (tf.float64,tf.float64))))
    dataset = dataset.map(
    ...
    features.set_shape((None,50,50,13))
    labels.set_shape((None,5000))
    #(features, labels) = (next_element[0],next_element[1])
    #batch_features = tf.cast(features,tf.float32)
    #batch_labels = tf.cast(labels,tf.float32)
    #[::2,::2,:]
    print("Here")
    return {'x': features}, labels


def convolve_wildfire_test(files,modelFnc,batchSize=100,model_dir="../models/wf_model"):
    ...
    # Test the model
    pred_results = classifier.predict(input_fn=lambda:myInputFn(files,batchSize,1))
...
def readSpecH5(specName):
    ...
    output[outputBurnmap > 0,1] = 1
    output = np.reshape(output,(5000,))
...
if __name__ == "__main__":
    inDir = 'E:/projects/wildfire-research/farsite/results/lowres/'
    files = glob.glob(inDir+'run*.h5')
    modelDir = "../models/farsiteTest_lowres_2"
    modelFnc = cnnModel3
    batchSize = 50
    epochs = 1
    ...
    with tf.Session() as sess:
        ...
        for i in range(0,epochs):
            ...
            with tf.variable_scope("cnnModelFS3") as scope:
                ...
                datas, truths = readAllData(files)
                ...
                #data, truth = readSpecH5(bytes(files[99],'utf-8'))
                #convolve_wildfire_train(files,modelFnc,epochs=2,batchSize=batchSize,model_dir=modelDir)
